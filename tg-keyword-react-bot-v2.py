...
KEYWORD_ACTIONS = {
    ...
    'naive': {'action': 'dm', 'sticker_pack': 'fuckgfwnewbie', 'sticker_index': 1},  # 第2个贴纸
}
...
class KeywordMonitorBot:
    ...
    async def send_sticker_reply(self, channel_id, message_id, sticker_file):
        ...
        try:
            ...
            logger.info(f"成功在 {channel_id} 的消息 {message_id} 回复贴纸")
            return True
        except Exception as e:
            logger.error(f"回复贴纸失败: {e}")
            return False
    ...
    async def handle_keyword_match(self, keyword, parsed_info):
        ...
        if action_type == 'reply':
            # 在源消息回复
            if parsed_info['source_channel'] and parsed_info['source_message_id']:
                return await self.send_sticker_reply(
                    parsed_info['source_channel'],
